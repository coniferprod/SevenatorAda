...
begin
    ...
    begin
        case Commands.Command_Type'Value (Command) is
            when Commands.Dump => Commands.Run_Dump (File_Name);
            when Commands.Cartridge => Commands.Run_Cartridge (File_Name);
            when Commands.Voice => Commands.Run_Voice (File_Name);
        end case;
    end;
end Sevenator;
