...
package body DX7 is
    ...
    function Get_Envelope_Data (Envelope: Envelope_Type) return Byte_Vectors.Vector is
    ...
    begin
        ...
        return BV;
    end Get_Envelope_Data;

    function Get_Operator_Data (Operator: Operator_Type) return Helpers.Byte_Vectors.Vector is
        BV : Byte_Vectors.Vector;
    begin
        BV.Append (Get_Envelope_Data (Operator.EG));
        return BV;
    end Get_Operator_Data;

    function Get_Voice_Data (Voice: Voice_Type) return Helpers.Byte_Vectors.Vector is
        BV : Byte_Vectors.Vector;
    begin
        for Op in Operator_Index loop
            BV.Append (Get_Operator_Data (Voice.Operators (Op)));
        end loop;
        return BV;
    end Get_Voice_Data;
